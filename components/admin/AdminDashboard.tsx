...
export default function AdminDashboard() {
  ...
  return (
    <div className="min-h-screen flex bg-slate-50 dark:bg-[#0d1b30]">
      {/* Backdrop for the mobile nav drawer - the sidebar itself is fixed off-canvas
          below md, sliding in over the content instead of squeezing it. */}
      {mobileNavOpen && (
      ...
      {/* Sidebar */}
      <aside
        className={`w-64 flex-shrink-0 fixed md:sticky top-0 h-screen border-r border-slate-200 dark:border-slate-800/70 bg-white dark:bg-slate-900 flex flex-col z-50 transition-transform duration-200 md:translate-x-0 ${
          mobileNavOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        <div className="h-14 flex items-center justify-between px-5 border-b border-slate-100 dark:border-slate-800/70">
          <Link
            href="/"
          ...
          <button
            ...
            className="md:hidden text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 text-lg leading-none"
          >
            &times;
          </button>
        </div>
      ...
      <div className="flex-1 min-w-0">
        {/* Mobile top bar - the sidebar (and its Save button) is off-canvas below md,
            so this exposes the menu toggle and current section, plus a reachable
            Save button, without it. */}
        <div className="md:hidden sticky top-0 z-30 flex items-center gap-3 h-14 px-4 border-b border-slate-200 dark:border-slate-800/70 bg-white dark:bg-slate-900">
          <button
            type="button"
            onClick={() => setMobileNavOpen(true)}
            aria-label="Open menu"
            className="text-slate-500 dark:text-slate-400 text-lg leading-none"
          >
            <i className="fa-solid fa-bars" />
          </button>
          <span className="flex-1 min-w-0 truncate text-sm font-semibold text-slate-800 dark:text-slate-100 flex items-center gap-2">
          ...
          <button
            ...
            onClick={handleSave}
            disabled={saving}
            className="flex-shrink-0 flex items-center justify-center gap-1.5 px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-semibold rounded-lg transition-colors disabled:opacity-60"
          >
            <i className="fa-solid fa-floppy-disk text-xs" /> {saving ? "Saving..." : "Save"}
          </button>
        </div>
        {saveState && (
          <p className={`md:hidden px-4 pt-2 text-xs ${saveState.ok ? "text-emerald-600 dark:text-emerald-400" : "text-red-500"}`}>
            {saveState.text}
          </p>
        )}
        ...
        <div className="px-4 py-6 sm:px-8 sm:py-8">
          <div className="max-w-4xl mx-auto">
            <div className="mb-6">
              ...
              <h2 className="hidden md:flex text-xl font-semibold text-slate-800 dark:text-slate-100 items-center gap-2 mt-1">
                <i className={`fa-solid ${current.icon} ${current.color}`} />
                {current.label}
              </h2>
              {current.description && <p className="text-sm text-slate-500 dark:text-slate-400 mt-1.5 max-w-2xl">{current.description}</p>}
            </div>
            <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-sm p-4 sm:p-6">{Comp}</div>
          </div>
        </div>
      </div>
    </div>
