...
export default function Dashboard({
  ...
  const hasIntegrations =
    (storage?.powerstores?.length ?? 0) > 0 ||
    (storage?.proxmoxes?.length ?? 0) > 0 ||
    (pbs?.targets?.length ?? 0) > 0 ||
    (integrations?.targets?.length ?? 0) > 0;
      ...
      <TopNav
        ...
        rightExtra={
          hasIntegrations ? (
            ...
            >
              <i className="fa-solid fa-store text-xs" /> Integrations
            </Link>
          ) : undefined
        }
      />
