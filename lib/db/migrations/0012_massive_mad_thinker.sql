...
INSERT INTO `__new_pbs_acknowledged_tasks`("id", "target_id", "task_id", "acknowledged_at")
SELECT "id", "target_id" + 300000, "task_id", "acknowledged_at" FROM `pbs_acknowledged_tasks`;--> statement-breakpoint
...
CREATE UNIQUE INDEX `uniq_pbs_target_task` ON `pbs_acknowledged_tasks` (`target_id`,`task_id`);--> statement-breakpoint
